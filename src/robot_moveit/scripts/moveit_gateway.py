...
class MoveItGateway(Node):
    ...
    def quaternion_multiply(q1, q2):
        """
        四元数乘法: q = q1 * q2 (numpy简化版)
        四元数格式: [x, y, z, w]
        """
        # 通过旋转矩阵相乘实现四元数乘法（更直观且利用numpy矩阵运算）
        # R_result = R_q1 @ R_q2, 然后转回四元数
        # 直接使用公式更快，这里用numpy向量化实现
        q1 = np.array(q1)
        q2 = np.array(q2)

        w1, x1, y1, z1 = q1[3], q1[0], q1[1], q1[2]  # [w, x, y, z] 顺序
    ...
    def quaternion_conjugate(q):
        """
        四元数共轭: q* = [ -x, -y, -z, w ] (numpy简化版)
        """
        q = np.array(q)
        return tuple(-q[:3].tolist() + [q[3]])  # 取反前三个元素，保持w不变
    ...
    def rotation_matrix_to_quaternion(R):
        """
        旋转矩阵转四元数 (numpy简化版)
        R: 3x3旋转矩阵 (numpy array or nested list)
        返回: [x, y, z, w]
        """
        # 确保是numpy数组
