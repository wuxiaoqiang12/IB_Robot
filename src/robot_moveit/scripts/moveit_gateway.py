...
v7: Thread-safe Multi-threaded execution model.
Fixed "wait set index too big" by using MultiThreadedExecutor and async IK calls.
v8: Added orientation projection to XZ plane for 5-DOF IK constraint.
"""
...
class MoveItGateway(Node):
    def __init__(self):
        ...
        self.declare_parameter('joint_names', ['1', '2', '3', '4', '5'])
        self.declare_parameter('shoulder_link', 'shoulder')
        ...
        self.get_logger().info(f"Initializing MoveIt Gateway (v7) for SO101...")
    ...
    def constrain_to_z_axis_only(self, quat):
        ...
        # 3. 归一化Z轴
        z_norm = math.sqrt(z_axis[0]**2 + z_axis[1]**2 + z_axis[2]**2)
        if z_norm > 1e-6:
        ...
        # 4. 构造新的X轴（在垂直于Z轴的平面内选择一个合理的方向）
        # 使用"最小旋转"原则：选择与原X轴最接近的方向
        orig_x_axis = (R[0][0], R[1][0], R[2][0])

        # 将原X轴投影到垂直于Z轴的平面上
        # proj = x - (x·z) * z
        dot_xz = orig_x_axis[0] * z_axis[0] + orig_x_axis[1] * z_axis[1] + orig_x_axis[2] * z_axis[2]
        x_axis = (
            orig_x_axis[0] - dot_xz * z_axis[0],
            orig_x_axis[1] - dot_xz * z_axis[1],
            orig_x_axis[2] - dot_xz * z_axis[2]
        )
        ...
        if x_norm > 1e-6:
            ...
            # 如果X轴退化（与Z轴平行），使用world的X轴投影
            if abs(z_axis[2]) < 0.9:
                # Z轴不是垂直的，使用垂直于Z轴的水平方向
                z_xy_norm = math.sqrt(z_axis[0]**2 + z_axis[1]**2)
                x_axis = (-z_axis[1]/z_xy_norm, z_axis[0]/z_xy_norm, 0.0)
            else:
                # Z轴垂直，X轴指向world的X方向
                x_axis = (1.0, 0.0, 0.0)
        ...
        # 6. 重建旋转矩阵（列存储：X轴是第0列，Y轴是第1列，Z轴是第2列）
        R_constrained = [
    ...
    def project_orientation_to_shoulder_xz_plane(self, quat):
        ...
        try:
            ...
            # 提取变换的四元数
            t_x = transform.transform.rotation.x
            t_y = transform.transform.rotation.y
            t_z = transform.transform.rotation.z
            t_w = transform.transform.rotation.w
            base_to_shoulder_q = (t_x, t_y, t_z, t_w)
        ...
        # 1. 将方向从base转换到shoulder坐标系
        # q_shoulder = q_base_to_shoulder * q_base
        ...
        # 3. 在shoulder坐标系中，约束在XZ平面
        # 提取旋转矩阵的三个列向量（即shoulder坐标系中旋转后的三个轴）
        x_axis = (R_shoulder[0][0], R_shoulder[1][0], R_shoulder[2][0])
        y_axis = (R_shoulder[0][1], R_shoulder[1][1], R_shoulder[2][1])
        z_axis = (R_shoulder[0][2], R_shoulder[1][2], R_shoulder[2][2])

        # 约束：X轴和Z轴应该在XZ平面内（Y分量为0）
        x_axis_constrained = (x_axis[0], 0.0, x_axis[2])
        z_axis_constrained = (z_axis[0], 0.0, z_axis[2])

        # 归一化
        x_norm = math.sqrt(x_axis_constrained[0]**2 + x_axis_constrained[2]**2)
        z_norm = math.sqrt(z_axis_constrained[0]**2 + z_axis_constrained[2]**2)

        if x_norm > 1e-6:
            x_axis = (x_axis_constrained[0]/x_norm, 0.0, x_axis_constrained[2]/x_norm)
        else:
            x_axis = (1.0, 0.0, 0.0)

        if z_norm > 1e-6:
            z_axis = (z_axis_constrained[0]/z_norm, 0.0, z_axis_constrained[2]/z_norm)
        else:
            z_axis = (0.0, 0.0, 1.0)
        ...
        # 5. 转换回base坐标系
        # q_base = q_shoulder_to_base * q_shoulder_constrained
